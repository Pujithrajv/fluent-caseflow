...
interface InterrogatoriesStepProps {
  data: any;
  cutoffDate: string;
...
export function InterrogatoriesStep({ data, cutoffDate, onNext, onBack }: InterrogatoriesStepProps) {
  ...
  const [replyDueDate, setReplyDueDate] = useState(data?.replyDueDate || "");
  ...
  const handleReceiveDateChange = (date: string) => {
    setReceiveDate(date);
    // Auto-calculate reply due date (28 days default)
    if (date) {
      const receive = new Date(date);
      receive.setDate(receive.getDate() + 28);
      setReplyDueDate(receive.toISOString().split('T')[0]);
    }
  };

  const showCutoffWarning = replyDueDate && new Date(replyDueDate) > new Date(cutoffDate);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Interrogatories</CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="replyDueDate">
              When is reply due? <span className="text-red-500">*</span>
            </Label>
            <Input
              id="replyDueDate"
              type="date"
