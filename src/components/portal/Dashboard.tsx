...
export function Dashboard({
  ...
  const filteredAndSortedEvents = useMemo(() => {
    ...
    if (eventsSearchQuery.trim()) {
      ...
      filtered = filtered.filter(event => event.title.toLowerCase().includes(query) || event.caseNumber.toLowerCase().includes(query) || event.caseType.toLowerCase().includes(query) || event.department.toLowerCase().includes(query) || event.primaryParty.toLowerCase().includes(query) || event.type.toLowerCase().includes(query) || event.location.toLowerCase().includes(query) || event.date.includes(query) || event.time.toLowerCase().includes(query) || event.meetingId && event.meetingId.includes(query));
    }

    // Apply type filters
    if (selectedTypeFilters.length > 0) {
      filtered = filtered.filter(event => selectedTypeFilters.includes(event.type));
    }
  ...
  const toggleTypeFilter = (type: string) => {
    setSelectedTypeFilters(prev => prev.includes(type) ? prev.filter(t => t !== type) : [...prev, type]);
  };
          ...
          <TabsContent value="cases" className="mt-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-2">
                <Button size="lg" className="font-fluent bg-blue-600 hover:bg-blue-700 text-white" onClick={() => navigate('/demo-request')}>
                  Demo Request
                </Button>
                
                <Button size="lg" className="font-fluent bg-blue-600 hover:bg-blue-700 text-white" onClick={() => navigate('/crm')}>
                  CRM
                </Button>
                
                <Button size="lg" className="font-fluent bg-blue-600 hover:bg-blue-700 text-white" onClick={() => navigate('/discoverys')}>
                  Discoverys
                </Button>
                
                 <Button size="lg" className="font-fluent bg-blue-600 hover:bg-blue-700 text-white" onClick={() => navigate('/submission-fillings')}>
                   Submission & Fillings
                 </Button>
              ...
              <div className="flex items-center space-x-2">
                <div className="relative">
                  ...
                  <Input placeholder="Search cases..." className="pl-10 w-64 h-11 font-fluent border-gray-400 bg-gray-50 focus:ring-primary" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-border">
                      ...
                      return <tr key={caseItem.id} className="hover:bg-muted/50 transition-colors">
                            ...
                            <td className="px-4 py-4 align-top">
                              <div className="space-y-1">
                                <p className="text-sm font-medium text-foreground">
                                  {caseItem.status === 'accepted' || caseItem.status === 'complete' || caseItem.status === 'closed' ? caseItem.caseNumber ? `Case #: ${caseItem.caseNumber}` : '—' : caseItem.confirmationNumber ? `Confirmation #: ${caseItem.confirmationNumber}` : '—'}
                                </p>
                            ...
                            <td className="px-4 py-4 align-top">
                              <div className="space-y-2">
                                {/* Next Event */}
                                {caseItem.nextEvent ? <div className="text-xs">
                                  ...
                                  </div> : <p className="text-xs text-muted-foreground">No events scheduled</p>}

                                {/* Deadlines (top 2) */}
                                {caseItem.deadlines?.slice(0, 2).map((deadline, index) => <div key={index} className="text-xs text-muted-foreground">
                                    <p>{deadline.name} — {formatDate(deadline.date)}</p>
                                  </div>)}

                                {/* Draft specific content */}
                                {caseItem.status === "draft" && <div className="space-y-2">
                                    <div className="text-xs text-muted-foreground">
                                      Draft saved — {formatDate(caseItem.lastActionDate)}
                                    </div>
                                    {onEditCase && <Button variant="outline" size="sm" onClick={() => onEditCase(caseItem.id, caseItem.lastWizardTab)} className="text-xs h-7 px-2">
                                        Continue Editing
                                      </Button>}
                                  </div>}
                              </div>
