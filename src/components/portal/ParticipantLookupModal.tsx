...
export function ParticipantLookupModal({
  ...
  const filteredParticipants = useMemo(() => {
    return currentParticipants.filter(participant => {
      ...
      return participant.name.toLowerCase().includes(searchLower) || participant.code.toLowerCase().includes(searchLower) || participant.classification.toLowerCase().includes(searchLower) || participant.participationType.toLowerCase().includes(searchLower) || participant.parentAccount.toLowerCase().includes(searchLower) || participant.caseDetails.toLowerCase().includes(searchLower);
    });
  ...
  const handleNewParticipantSave = async (data: any) => {
    ...
    try {
      const newParticipant: Participant = {
        ...
        name: data.firstName && data.lastName ? `${data.firstName.trim()} ${data.lastName.trim()}`.trim() : data.organization?.trim() || "New participant",
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[80vh] bg-slate-100">
        <DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <Input placeholder="Search by name, type, or case..." className="pl-10" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
          </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg overflow-hidden max-h-96 overflow-y-auto">
            ...
            <RadioGroup value={selectedParticipantId?.toString()} onValueChange={value => setSelectedParticipantId(parseInt(value))}>
              <div className="divide-y divide-gray-200">
                {filteredParticipants.map(participant => <div key={participant.id} className="px-4 py-4 bg-white hover:bg-gray-50 transition-colors cursor-pointer" onDoubleClick={() => handleRowDoubleClick(participant)}>
                    <div className="grid grid-cols-6 gap-4 items-center">
                      <div className="flex items-center space-x-3">
                        <RadioGroupItem value={participant.id.toString()} id={`participant-${participant.id}`} />
                        <Label htmlFor={`participant-${participant.id}`} className="font-medium text-gray-900 cursor-pointer text-sm">
                          {participant.code}
                        </Label>
                      </div>
                ...
                {filteredParticipants.length === 0 && <div className="px-4 py-8 text-center text-gray-500">
        ...
        <div className="flex justify-between items-center pt-4 border-t">
          <div className="flex space-x-2">
            <Button variant="outline" onClick={() => setIsNewParticipantModalOpen(true)}>
          ...
          <div className="flex space-x-2">
            {currentValue && onRemoveValue && <Button variant="destructive" onClick={onRemoveValue}>
                Remove value
              </Button>}
            <Button variant="outline" onClick={onClose}>
              Cancel
            </Button>
            <Button onClick={handleSelect} disabled={!selectedParticipantId}>
              Select
            </Button>
          </div>
        </div>
      ...
      <Dialog open={isNewParticipantModalOpen} onOpenChange={setIsNewParticipantModalOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto" aria-labelledby="new-participant-title">
          <DialogHeader>
          ...
          <Form {...newParticipantForm}>
            <form onSubmit={newParticipantForm.handleSubmit(handleNewParticipantSave)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Left Column */}
                <div className="space-y-4">
                  <FormField control={newParticipantForm.control} name="firstName" render={({
                  field
                }) => <FormItem>
                  ...
                  <FormField control={newParticipantForm.control} name="lastName" render={({
                  field
                }) => <FormItem>
                  ...
                  <FormField control={newParticipantForm.control} name="roleTitle" render={({
                  field
                }) => <FormItem>
                  ...
                  <FormField control={newParticipantForm.control} name="organization" render={({
                  field
                }) => <FormItem>
                ...
                {/* Right Column */}
                <div className="space-y-4">
                  <FormField control={newParticipantForm.control} name="phone" render={({
                  field
                }) => <FormItem>
                        <FormLabel>Phone</FormLabel>
                        <FormControl>
                          <Input {...field} placeholder="(555) 123-4567" onChange={e => handlePhoneChange(e.target.value)} />
                        </FormControl>
                      </FormItem>} />

                  <FormField control={newParticipantForm.control} name="email" render={({
                  field
                }) => <FormItem>
                  ...
                  <FormField control={newParticipantForm.control} name="address" render={({
                  field
                }) => <FormItem>
                  ...
                  <FormField control={newParticipantForm.control} name="notes" render={({
                  field
                }) => <FormItem>
              ...
              <div className="flex justify-end space-x-2 pt-4 border-t">
                <Button type="button" variant="outline" onClick={handleNewParticipantCancel} disabled={isSubmitting}>
                  Cancel
                </Button>
