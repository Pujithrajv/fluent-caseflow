...
export function DocumentProductionQuestionsTab({ 
  data, 
  onDataChange, 
  onValidationChange, 
  isReadOnly = false 
}: DocumentProductionQuestionsTabProps) {
  const [formData, setFormData] = useState({
    persons: data.persons || [{ id: '1', name: '', phone: '', email: '' }],
    receiveDate: data.receiveDate ? new Date(data.receiveDate) : undefined,
    productionDueDate: data.productionDueDate ? new Date(data.productionDueDate) : undefined,
    ...data
  });
  ...
  const validateForm = () => {
    ...
    const hasProductionDueDate = !!formData.productionDueDate;
    
    const isValid = hasValidPersons && hasReceiveDate && hasProductionDueDate;
    ...
    <Card className="shadow-fluent-8">
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <Label className="font-fluent text-base font-medium">
              What persons to receive a Request to Produce *
            </Label>
            <Button
              type="button"
              variant="outline"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            <Label className="font-fluent">When is production due *</Label>
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  ...
                  className={cn(
                    "w-full justify-start text-left font-normal shadow-fluent-8 border-input-border",
                    !formData.productionDueDate && "text-muted-foreground"
                  )}
                >
                  <CalendarIcon className="mr-2 h-4 w-4" />
                  {formData.productionDueDate ? format(formData.productionDueDate, "PPP") : <span>Pick a date</span>}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar
                  mode="single"
                  selected={formData.productionDueDate}
                  onSelect={(date) => updateFormData({ productionDueDate: date })}
                  initialFocus
                  className="p-3 pointer-events-auto"
                />
              </PopoverContent>
